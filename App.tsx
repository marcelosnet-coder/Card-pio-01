...
import { ShoppingCart, ArrowLeft, Trash2, Check, Plus, Repeat, Send, MapPin, Phone, User, CreditCard, Banknote, QrCode } from 'lucide-react';
...
const ProductCard: React.FC<{ 
  ...
  <motion.div 
    ...
    className="bg-zinc-900 rounded-xl overflow-hidden shadow-lg border border-zinc-800 cursor-pointer flex flex-col"
  >
    ...
    <div className="p-4 flex-1 flex flex-col justify-between">
      ...
      <div className="mt-4 flex items-center justify-between">
        ...
        <button className="bg-orange-600 hover:bg-orange-700 text-white px-3 py-1 rounded-full text-xs font-black uppercase flex items-center gap-1">
          <Plus size={14} />
          Adicionar
        </button>
      </div>
...
export default function App() {
  ...
  return (
    <div className="min-h-screen pb-24 max-w-md mx-auto relative bg-[#0f0f0f]">
        ...
        <AnimatePresence mode="wait">
          {view === 'menu' && (
            ...
            >
              ...
              <section>
                <h2 className="text-lg font-black mb-4 border-l-4 border-orange-600 pl-3 uppercase italic flex items-center gap-2">
                  Pizzas
                </h2>
              ...
              <section>
                <h2 className="text-lg font-black mb-4 border-l-4 border-yellow-500 pl-3 uppercase italic flex items-center gap-2">
                  Combos
                </h2>
                <div className="grid grid-cols-1 gap-6">
                  {MENU_COMBOS.map(p => (
              ...
              <section>
                <h2 className="text-lg font-black mb-4 border-l-4 border-white pl-3 uppercase italic flex items-center gap-2">
                  Bebidas
                </h2>
                <div className="grid grid-cols-1 gap-6">
          ...
          {view === 'selection' && (
            <motion.div 
              ...
              className="fixed inset-0 z-50 bg-[#0f0f0f] p-6 overflow-y-auto"
            >
               <div className="flex items-center justify-between mb-8">
                 <h2 className="text-2xl font-black text-orange-500 uppercase italic">Personalizar</h2>
                 <button onClick={() => setView('menu')} className="text-zinc-500 font-bold flex items-center gap-1">
                   <ArrowLeft size={18} />
                   Voltar
                 </button>
               </div>
     ...
     <div className="fixed bottom-0 left-0 right-0 p-6 bg-black/90 backdrop-blur-sm border-t border-zinc-800">
       <div className="max-w-md mx-auto space-y-4">
         ...
         <button 
           ...
           className="w-full bg-orange-600 disabled:bg-zinc-800 disabled:text-zinc-600 text-white font-black py-4 rounded-xl uppercase tracking-widest text-xs flex items-center justify-center gap-2"
         >
           <Check size={16} />
           Confirmar Pizza
         </button>
...
{view === 'checkout' && (
  ...
  >
    ...
    <div className="bg-zinc-900 rounded-2xl p-6 border border-zinc-800">
      <h3 className="text-zinc-500 font-black uppercase text-[10px] tracking-widest mb-4 flex items-center gap-2">
      ...
      <div className="space-y-4">
        {cart.map(item => (
          <div key={item.id} className="flex justify-between items-start border-b border-zinc-800 pb-3">
            ...
            <button onClick={() => removeFromCart(item.id)} className="text-red-500 font-bold hover:bg-red-500/10 p-1 rounded-lg transition-colors">
              <Trash2 size={18} />
            </button>
          </div>
        ))}
    ...
    <div className="space-y-4">
       <h3 className="text-zinc-500 font-black uppercase text-[10px] tracking-widest flex items-center gap-2">
         <MapPin size={14} />
         Entrega:
       </h3>
       <div className="relative">
         ...
         <input 
           ...
           className="w-full bg-zinc-900 border border-zinc-800 rounded-xl p-4 pl-12 focus:outline-none focus:ring-1 focus:ring-orange-600"
         />
       </div>
       <div className="relative">
       ...
       <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            {[
              { name: 'Dinheiro', icon: <Banknote size={14} /> },
              { name: 'Pix', icon: <QrCode size={14} /> },
              { name: 'Cartão de Crédito', icon: <CreditCard size={14} /> },
              { name: 'Cartão de Débito', icon: <CreditCard size={14} /> }
            ].map((method) => (
              <button
                key={method.name}
                onClick={() => setCustomer({ ...customer, paymentMethod: method.name })}
      ...
      {view === 'menu' && cart.length > 0 && (
        <div className="fixed bottom-6 left-6 right-6 z-40 max-w-md mx-auto">
          <motion.button 
            ...
            className="w-full bg-orange-600 hover:bg-orange-700 text-white p-4 rounded-2xl shadow-2xl flex items-center justify-between"
          >
            <div className="flex items-center">
              <span className="bg-white text-orange-600 font-black w-7 h-7 flex items-center justify-center rounded-lg mr-3 text-xs">
                {cart.length}
              </span>
              <span className="font-black tracking-widest uppercase text-[10px] flex items-center gap-2">
                <ShoppingCart size={14} />
                Ver Carrinho
              </span>
            </div>
